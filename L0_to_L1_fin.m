%% L0 to L1 data. L1 data will have pre and post deployment data re,oved and QC applied. 
% need to load m file 
% Use L0 mat file
% for multiple deployments i = the deployment number

load('ADCP_L0_processing.mat')

%% section 1, finding the time the ADCP is in and out the water

% use depth field first to try and determine pre and post deployment data

for i = 1:length(ADCP)
       clear bad_depth
       if length(unique(ADCP(i).depth))>2 && mean(ADCP(i).depth) > 23 %Checking to make sure pressure sensor is recording and in the range of what we expect
...
for i = 1:length(ADCP)
       clear bad_echo
           ...
           bad_echo = find(ADCP(i).intens(25,1,[1:15 end-15:end]) < 90); %used to index values above the surface. only looking in bin 25
...
%% Section 2, Manual input to determine pre and post
...
%% Section 3, Get rid of bad pressure
...
msgbox('Section 4 complete. If any of the graphs are not consistent, proceed to Section 5')
...
%% Section 6, Applying QC procedures
% north values
% velocity error test with a throshold of 0.05 cm/s
for zz=1:length(ADCP)
    clear col dif ind row xx yy 
    [row,col] = find(abs(ADCP(zz).error_vel)>=0.05); %indexes bin and ensemble for which the velocity error is above threshold
    ...
    for xx=1:length(ADCP(zz).mtime)
        [row,col] = find(ADCP(zz).corr(:,:,xx)<=110); %indexes values that fail
         for yy= 1:length(row)
...
msgbox('Section 6 complete')
%% Section 7, East values
% same as the above procedures only now the east velocity values are
% screened
for zz=1:length(ADCP)
    clear col dif ind row xx yy 
    [row,col] = find(abs(ADCP(zz).error_vel)>=0.05);
    ...
    for xx=1:length(ADCP(zz).mtime)
        [row,col] = find(ADCP(zz).corr(:,:,xx)<=110);
         for yy= 1:length(row)
...
%% Section 8, Calculating depth from pressure data
...
%% Section 9, calculating 4 beam average EA
...
for zz = 1:length(ADCP)
    ...
    for xx=1:length(ADCP(zz).mtime)
        for yy=15:ADCP(zz).config.n_cells-1 %only bins above the 15th bin
...
%% Section 10, Remove fields that are not needed for L1
...
%% Section 11, Removing values that are above Sea surface 
...
save ADCP_L1 ADCP
msgbox('Section 11 complete. You have successfully processed data to the L1 tier')
clear